...
class CartViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if cart_items:  # Если продукт уже есть в корзине
            ...
            product = get_object_or_404(Product, id=request.data.get('product'))  # Получаем продукт и проверяем что он вообще существует, если его нет то выйдет ошибка 404
...
class WishlistView(View):
    ...
    def get(self, request):
        if request.user.is_authenticated:
            list_of_data = self.get_queryset()
